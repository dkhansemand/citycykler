<?php
    ...
    if(isset($POST['btnSubmit'])){
        ...
        if(sizeof($error) === 0){
            Offer::Edit(Router::GetParam(':OID'), $productId, $offerPrice);
            $success = 'Tilbud er blevet rettet!';
            unset($POST);
        }else{
            $addError = 'Der skete en fejl.';
        }
    }
    $offer = Offer::GetOfferById(Router::GetParam(':OID'));
?>
<form action="" method="post" enctype="multipart/form-data">
<input type="hidden" name="url" id="siteUrl" value="<?=Router::Link('/OfferAdd/' . $offer->offerId)?>">
    <div class="mdl-grid">
        ...
        <div class="mdl-cell mdl-cell--12-col">
            ...
            <?php
                ...
                {
            ?>
                    <option value="<?=$model->productId?>" <?= (@$POST['productModel'] || $offer->productModel == $model->productModel) ? 'selected' : ''?> ><?=$model->productModel?></option>
            <?php
                }          
            ?>
            </select>    
        ...
        <div class="mdl-cell mdl-cell--12-col">
            <div class="mdl-textfield mdl-js-textfield mdl-textfield--floating-label">
                <input class="mdl-textfield__input" name="offerPrice" type="text" pattern="-?[0-9]*(\.[0-9]+)?" id="offerPrice"  value="<?=@$POST['offerPrice'] ?? $offer->offerPrice?>" required>
        ...
        <div class="mdl-cell mdl-cell--12-col">
            <button name="btnSubmit" class="mdl-button mdl-js-button mdl-button--raised mdl-button--colored">
                <i class="material-icons">edit</i> Ret tilbud
            </button>
        </div>
            <?php }?>
    </div>
